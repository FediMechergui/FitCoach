/**
 * The report document itself — pure HTML from ReportData, no native imports, so
 * it can be rendered and checked in Node against a full and a sparse fixture.
 * pdfReport.ts turns it into a PDF and opens the share sheet.
 */
import type { ReportData } from '@/repositories/reportRepo';
...
export function buildReportHtml(d: ReportData): string {
  ...
  const nutrition = n
    ? section(
        ...
        kv([
          ...
          ['Fibre target', `${n.fiberTargetG} g`],
          ['Avg fibre (7d / 30d)', `${n.avg7d.fiber} / ${n.avg30d.fiber} g`],
  ...
  const mi = d.micros;
  const microsSec = mi.daysWithData > 0
    ? section(
        'Micronutrients (7-day average)',
        `<p class="muted">From ${mi.daysWithData} day${mi.daysWithData === 1 ? '' : 's'} with data · ${mi.coveredCount} of ${mi.trackedCount} tracked nutrients at or above the reference intake.</p>` +
          (mi.gaps.length
            ? `<table class="data"><tr><th>Running low</th><th>Avg / day</th><th>Reference</th><th>% RDI</th></tr>${mi.gaps
                .map((g) => `<tr><td>${esc(g.label)}</td><td>${esc(g.avgAmount)}</td><td>${esc(g.rdi)}</td><td>${g.pct}%</td></tr>`)
                .join('')}</table>`
            : '<p class="muted">Nothing under 50% of the reference intake this week.</p>') +
          '<p class="disclaimer">Food micronutrients are per-serving reference values summed from what was logged; supplements are counted from their labels. An estimate of intake, not a blood test.</p>'
      )
    : '';

  const supps = d.supplements.length
    ? section(
        'Supplement Stack',
        `<ul>${d.supplements.map((s) => `<li><strong>${esc(s.label)}</strong> — ${esc(s.dose)} <span class="muted">(${esc(s.category)})</span></li>`).join('')}</ul>`
      )
    : '';
  ...
  const order = isCoach
    ? [profile, rating, training, sleep, bodyComp, alcohol, smoking, cycle, nutrition, supps, conditions]
    : [profile, nutrition, microsSec, supps, bodyComp, alcohol, smoking, sleep, cycle, training, rating, conditions];
